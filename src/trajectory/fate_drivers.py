...
from trajectory_utils import (
    RESULT_DIR,
    load_annotated, save_figure, save_table, set_seed, style_bar, style_dark,
)
...
HEATMAP_FIGSIZE = (9, 18)
GRID_COLUMNS    = 3
PANEL_SIZE      = (6, 5)


def load_fates(index: pd.Index) -> pd.DataFrame:
    """Read the stored fate probability matrix and its lineage names."""
    ...
    print(f"loaded fate probabilities {matrix.shape} for {FATE_TAG}")
...
def top_drivers(correlations: pd.DataFrame, pvalues: pd.DataFrame,
                fraction: pd.Series) -> pd.DataFrame:
...
def plot_driver_heatmap(correlations: pd.DataFrame) -> None:
    ...
    axis.set_title("Fate probability correlation of the leading driver genes")
...
def main() -> None:
    """Correlate gene expression with fate probability and store the driver tables and figures."""
    set_seed()
